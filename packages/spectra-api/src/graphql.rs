    ...
    node.to_string_vec()
        ...
        .map(|segment| {
            segment
                ...
                .unwrap_or(PathSegment::Field(segment))
        })
        .collect()
}
...
impl fmt::Debug for GraphqlWorkerExecutor {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GraphqlWorkerExecutor")
...
impl GraphqlWorkerExecutor {
    ...
    pub fn new(max_concurrency: usize) -> Self {
        assert!(
            max_concurrency > 0,
            "GraphQL worker concurrency must be non-zero"
        );
        Self {
    ...
    fn dispatch(&self, callback: ResolverCallback, input: ResolverInput) -> ResolverFuture {
        ...
        Box::pin(async move {
            let permit = permits
                .acquire_owned()
                .await
                .map_err(|_| Error::new("GraphQL worker executor closed"))?;
...
fn worker_join_error(err: JoinError) -> Error {
    if err.is_cancelled() {
        Error::new("GraphQL resolver worker cancelled")
    } else {
        Error::new(format!("GraphQL resolver worker failed: {err}"))
    }
}
...
pub type GraphqlSubscriptionCallback = Arc<
    dyn Fn(
            ResolverInput,
        )
            -> Pin<Box<dyn Future<Output = Result<SubscriptionStream, Error>> + Send + 'static>>
        + Send
        + Sync
        + 'static,
>;
...
impl GraphqlSchemaBuilder {
    ...
    #[must_use]
    pub fn context(mut self, context: Arc<GraphqlContext>) -> Self {
        self.context = context;
        self
    }

    #[must_use]
    pub fn workers(mut self, workers: GraphqlWorkerExecutor) -> Self {
    ...
    pub fn subscription_capacity(mut self, capacity: usize) -> Self {
        assert!(
            capacity > 0,
            "GraphQL subscription capacity must be non-zero"
        );
    ...
    #[must_use]
    pub fn register_type(mut self, ty: impl Into<Type>) -> Self {
        self.types.push(ty.into());
        self
    }
    #[must_use]
    pub fn register_scalar(self, ty: Scalar) -> Self {
        self.register_type(ty)
    }
    #[must_use]
    pub fn register_object(self, ty: Object) -> Self {
        self.register_type(ty)
    }
    #[must_use]
    pub fn register_input_object(self, ty: InputObject) -> Self {
        self.register_type(ty)
    }
    #[must_use]
    pub fn register_enum(self, ty: Enum) -> Self {
        self.register_type(ty)
    }
    #[must_use]
    pub fn register_interface(self, ty: Interface) -> Self {
        self.register_type(ty)
    }
    #[must_use]
    pub fn register_union(self, ty: Union) -> Self {
        self.register_type(ty)
    }
    ...
    pub fn finish(self) -> Result<GraphqlSchema, GraphqlSchemaError> {
        ...
        let schema = if let Some(mutation) = self.mutation {
            schema.register(mutation)
        } else {
            schema
        };
        let schema = if let Some(subscription) = self.subscription {
...
fn resolver_input(
    ...
    field_name: &str,
    default_context: &Arc<GraphqlContext>,
) -> ResolverInput {
...
fn dynamic_field(
            ...
            match future.await {
                ...
                Err(error) => {
                    if let Some(tracker) = tracker {
                        tracker
                            .record(error.message.clone(), path.unwrap_or_default())
                            .await;
                    }
                    Err(error)
                }
    ...
    for argument in arguments {
        dynamic = dynamic.argument(argument);
    }
    dynamic
}

fn dynamic_subscription_field(
    field: GraphqlSubscriptionField,
    workers: GraphqlWorkerExecutor,
    context: Arc<GraphqlContext>,
) -> SubscriptionField {
    let GraphqlSubscriptionField {
        name,
        ty,
        arguments,
        resolver,
...
impl fmt::Debug for GraphqlSchema {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GraphqlSchema").finish_non_exhaustive()
    }
}

impl GraphqlSchema {
    pub fn builder() -> GraphqlSchemaBuilder {
        GraphqlSchemaBuilder::default()
    }
    ...
    pub async fn execute(&self, request: impl Into<GraphqlRequest>) -> GraphqlResponse {
        ...
        let mut response = self
            .schema
            .execute(request.into_request(self.context.clone(), tracker.clone()))
            .await;
    ...
    pub fn subscribe(&self, request: impl Into<GraphqlRequest>) -> GraphqlSubscription {
        ...
        let stream = self
            .schema
            .execute_stream(request.into_request(self.context.clone(), tracker.clone()));
    ...
    pub fn worker_executor(&self) -> &GraphqlWorkerExecutor {
        &self.workers
    }
}
...
impl GraphqlRequest {
    ...
    fn into_request(
        ...
        request
            .data(self.context.unwrap_or(default_context))
            .data(tracker)
    }
}
...
impl GraphqlResponse {
    ...
    pub fn json_value(&self) -> serde_json::Value {
        serde_json::to_value(&self.0)
            .unwrap_or_else(|_| json!({"errors":[{"message":"response serialization failed"}]}))
    }
    pub fn json_bytes(&self) -> Vec<u8> {
        serde_json::to_vec(&self.0).unwrap_or_else(|_| {
            b"{\"errors\":[{\"message\":\"response serialization failed\"}]}".to_vec()
        })
    }
}
...
impl GraphqlSubscription {
    fn start(
        mut stream: BoxStream<'static, Response>,
        capacity: usize,
        tracker: Arc<ErrorPathStore>,
    ) -> Self {
...
impl Drop for GraphqlSubscription {
    fn drop(&mut self) {
        self.cancel();
    }
}
...
) -> Result<GraphqlRequest, GraphqlHttpError> {
    match *request.method() {
        Method::GET => {
            let query = request
                .uri()
                .query()
                .ok_or_else(|| GraphqlHttpError::InvalidQuery("missing query parameter".into()))?;
        ...
        Method::POST => {
            let content_type = request
                ...
                .unwrap_or("")
                .trim();
            if content_type != "application/json" && content_type != "application/graphql+json" {
                return Err(GraphqlHttpError::UnsupportedMediaType);
            }
            serde_json::from_slice(request.body())
                .map_err(|err| GraphqlHttpError::InvalidBody(err.to_string()))
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedMediaType => {
                f.write_str("GraphQL POST requires application/json or application/graphql+json")
            }
...
fn parse_get_query(query: &str) -> Result<GraphqlRequest, GraphqlHttpError> {
    ...
    if let Some(variables) = values.remove("variables") {
        request.variables = serde_json::from_str(&variables).map_err(|err| {
            GraphqlHttpError::InvalidQuery(format!("variables must be JSON: {err}"))
        })?;
    }
    Ok(request)
}
...
fn percent_decode(value: &str) -> Result<String, GraphqlHttpError> {
        ...
        match bytes[index] {
            ...
            b'%' if index + 2 < bytes.len() => {
                let high = hex(bytes[index + 1]).ok_or_else(|| {
                    GraphqlHttpError::InvalidQuery("invalid percent escape".into())
                })?;
            ...
            b'%' => {
                return Err(GraphqlHttpError::InvalidQuery(
    ...
    String::from_utf8(output)
        .map_err(|_| GraphqlHttpError::InvalidQuery("query parameters must be UTF-8".into()))
}
...
fn graphql_http_error(error: GraphqlHttpError) -> HttpResponse<Vec<u8>> {
    let status = match error {
        GraphqlHttpError::MethodNotAllowed => StatusCode::METHOD_NOT_ALLOWED,
        GraphqlHttpError::UnsupportedMediaType => StatusCode::UNSUPPORTED_MEDIA_TYPE,
        _ => StatusCode::BAD_REQUEST,
    };
    let body = serde_json::to_vec(&json!({"errors":[{"message": error.to_string()}]}))
        .unwrap_or_else(|_| b"{\"errors\":[{\"message\":\"bad request\"}]}".to_vec());
...
mod tests {
    ...
    async fn nested_alias_fragments_directives_and_variables() {
        let user =
            Object::new("User").field(Field::new("name", TypeRef::named_nn("String"), |_| {
                FieldFuture::from_value(Some(Value::from("Ada")))
            }));
        ...
        let schema = GraphqlSchema::builder()
            ...
            .field(
                ...
                GraphqlField::new(
                    "user",
                    TypeRef::named_nn("User"),
                    static_value_resolver(json_value(json!({"name": "Ada"}))),
                ),
            )
            .field(
                GraphqlRoot::Query,
                GraphqlField::new(
                    ...
                    static_value_resolver(Value::from(true)),
                ),
            )
            .field(
                GraphqlRoot::Query,
                GraphqlField::new("echo", TypeRef::named_nn("String"), echo).argument(
                    InputValue::new("prefix", TypeRef::named("String")).default_value("default"),
                ),
            )
            .finish()
            .unwrap();
    ...
    async fn mutation_fields_are_serial_and_callbacks_run_on_workers() {
        ...
        let schema = GraphqlSchema::builder()
            ...
            .field(
                GraphqlRoot::Mutation,
                field("first", worker_ok.clone(), order.clone()),
            )
            .field(
                GraphqlRoot::Mutation,
                field("second", worker_ok.clone(), order.clone()),
            )
            .finish()
            .unwrap();
    ...
    async fn max_depth_rejects_over_deep_queries() {
        ...
        let build = || {
            GraphqlSchema::builder().register_object(user()).field(
                GraphqlRoot::Query,
                GraphqlField::new(
                    "user",
                    TypeRef::named_nn("User"),
    ...
    async fn introspection_toggle_hides_schema() {
        ...
        let open = build().finish().unwrap();
        let visible = open
            .execute_query("{ __schema { queryType { name } } }")
            .await;
        assert_eq!(
            visible.json_value()["data"]["__schema"]["queryType"]["name"],
            "Query"
        );
    ...
    async fn introspection_and_validation_errors_preserve_locations_and_paths() {
        let schema = GraphqlSchema::builder()
            .field(
                ...
                GraphqlField::new(
                    ...
                    resolver_callback(|_| async { Err(Error::new("boom")) }),
                ),
            )
            .finish()
            .unwrap();
        let introspection = schema
            ...
            .await;
        assert_eq!(
            introspection.json_value()["data"]["__schema"]["queryType"]["name"],
            "Query"
        );
    ...
    async fn http_get_post_and_method_content_type_errors() {
        ...
        let request = HttpRequest::builder()
            ...
            .unwrap();
        assert_eq!(
            schema.execute_http(request).await.status(),
