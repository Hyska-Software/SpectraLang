...
fn resolver_from_callback(callback: CallbackEntry) -> graphql::ResolverCallback {
    graphql::resolver_callback(move |input: ResolverInput| {
        ...
        async move {
            ...
            let result = invoke_callback(callback, request).map_err(|status| {
                graphql::Error::new(format!(
                    "GraphQL resolver callback failed (status {status})"
                ))
            })?;
            let encoded = read_spectra_string(result).ok_or_else(|| {
                graphql::Error::new("GraphQL resolver callback must return a JSON string")
            })?;
            let value = serde_json::from_str::<JsonValue>(&encoded).map_err(|error| {
                graphql::Error::new(format!("GraphQL resolver returned invalid JSON: {error}"))
            })?;
            let value = json_to_graphql(value).ok_or_else(|| {
                graphql::Error::new(
                    "GraphQL resolver result cannot be represented as GraphQL value",
                )
            })?;
            Ok(Some(value))
        }
...
fn pull_batch(
    ...
    input: &ResolverInput,
) -> Result<Vec<graphql::Value>, graphql::Error> {
    let request = callback_input(input)
        .ok_or_else(|| graphql::Error::new("failed to encode GraphQL subscription input"))?;
    let result = invoke_callback(callback, request).map_err(|status| {
        graphql::Error::new(format!(
            "GraphQL subscription callback failed (status {status})"
        ))
    })?;
    let encoded = read_spectra_string(result).ok_or_else(|| {
        graphql::Error::new("GraphQL subscription callback must return a JSON array string")
    })?;
    let values = serde_json::from_str::<Vec<JsonValue>>(&encoded).map_err(|error| {
        graphql::Error::new(format!(
            "GraphQL subscription returned invalid JSON array: {error}"
        ))
    })?;
    values
        .into_iter()
        .map(|value| {
            json_to_graphql(value).ok_or_else(|| {
                graphql::Error::new(
                    "GraphQL subscription event cannot be represented as GraphQL value",
                )
            })
        })
        .collect()
...
fn parse_request(
    query: SpectraHostValue,
    variables: SpectraHostValue,
    operation: Option<SpectraHostValue>,
) -> Option<GraphqlRequest> {
...
fn insert_response(response: GraphqlResponseOwned, ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(handle) = store.responses.insert(ResponseEntry {
        json: response.json,
    ...
    F: FnOnce(GraphqlSchemaBuilder) -> GraphqlSchemaBuilder,
{
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(builder) = store.builders.get_mut(raw) else {
        return false;
    };
...
pub extern "C" fn schema_new(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(handle) = store.builders.insert(GraphqlSchemaBuilder::new()) else {
        return invalid();
    };
    write_result(ctx, handle)
}
...
pub extern "C" fn schema_set_workers(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Ok(concurrency) = usize::try_from(args[1]) else {
        return invalid();
    };
    if concurrency == 0 {
        return invalid();
    }
    if !update_builder(args[0], |builder| {
        builder.workers(graphql::GraphqlWorkerExecutor::new(concurrency))
    }) {
        return invalid();
    }
    write_result(ctx, 1)
...
pub extern "C" fn schema_set_subscription_capacity(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    if !update_builder(args[0], |builder| builder.subscription_capacity(capacity)) {
        return invalid();
    }
    write_result(ctx, 1)
}
...
pub extern "C" fn schema_set_max_depth(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    if !update_builder(args[0], |builder| builder.max_depth(depth)) {
        return invalid();
    }
    write_result(ctx, 1)
}
...
pub extern "C" fn schema_set_max_complexity(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    if !update_builder(args[0], |builder| builder.max_complexity(complexity)) {
        return invalid();
    }
    write_result(ctx, 1)
}
...
pub extern "C" fn schema_set_introspection(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    if !update_builder(args[0], |builder| builder.introspection(args[1] != 0)) {
        return invalid();
    }
    write_result(ctx, 1)
}
...
pub extern "C" fn schema_field_json(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(root) = root(args[1]) else {
        return invalid();
    };
    let (Some(name), Some(type_name), Some(encoded)) = (
    ...
    let Some(value) = json_to_graphql(value) else {
        return invalid();
    };
    let Some(ty) = graphql_type(type_name) else {
        return invalid();
    };
    if !update_builder(args[0], |builder| {
        builder.field(
            root,
            GraphqlField::new(name, ty, graphql::static_value_resolver(value)),
        )
    }) {
        return invalid();
    }
...
pub extern "C" fn schema_field_callback(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let (Some(name), Some(ty), Some(callback)) = (
        read_spectra_string(args[2]),
        read_spectra_string(args[3]).and_then(graphql_type),
        callback_entry(ctx, args[4]),
    ) else {
        return invalid();
    };
    if !update_builder(args[0], |builder| {
        builder.field(
            root,
            GraphqlField::new(name, ty, resolver_from_callback(callback)),
        )
    }) {
        return invalid();
    }
...
pub extern "C" fn schema_subscription_json(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Ok(values) = serde_json::from_str::<Vec<JsonValue>>(&encoded) else {
        return invalid();
    };
    let Some(ty) = graphql_type(type_name) else {
        return invalid();
    ...
    if !update_builder(args[0], |builder| {
        builder.subscription_field(GraphqlSubscriptionField::new(name, ty, callback))
    }) {
        return invalid();
    }
    write_result(ctx, 1)
...
pub extern "C" fn schema_subscription_callback(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let (Some(name), Some(ty), Some(callback)) = (
        read_spectra_string(args[1]),
        read_spectra_string(args[2]).and_then(graphql_type),
        callback_entry(ctx, args[3]),
    ) else {
        return invalid();
    };
    if !update_builder(args[0], |builder| {
        builder.subscription_field(GraphqlSubscriptionField::new(
            name,
            ty,
...
pub extern "C" fn schema_finish(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Ok(schema) = builder.finish() else {
        return invalid();
    };
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(handle) = store.schemas.insert(schema) else {
        return invalid();
    };
    write_result(ctx, handle)
}
...
pub extern "C" fn schema_drop(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    if store.schemas.remove(args[0]).is_some() || store.builders.remove(args[0]).is_some() {
        write_result(ctx, 1)
    } else {
        invalid()
    }
...
pub extern "C" fn schema_sdl(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(schema) = get_schema(args[0]) else {
        return invalid();
    };
...
fn execute_request(ctx: *mut SpectraHostCallContext, request: GraphqlRequest) -> i32 {
    ...
    let Some(response) = execute_on_runtime(async move { schema.execute(request).await }) else {
        return invalid();
    };
    let Ok(json) = String::from_utf8(response.json_bytes()) else {
        return invalid();
    };
...
pub extern "C" fn execute(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(request) = parse_request(args[1], args[2], None) else {
        return invalid();
    };
...
pub extern "C" fn execute_named(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(request) = parse_request(args[1], args[2], Some(args[3])) else {
        return invalid();
    };
    execute_request(ctx, request)
}
...
pub extern "C" fn execute_http(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(target) = read_spectra_string(args[2]) else {
        return invalid();
    };
    let Some(content_type) = read_spectra_string(args[3]) else {
        return invalid();
    };
    let Some(body) = read_spectra_string(args[4]) else {
        return invalid();
    };
    ...
    let Ok(header) = ::http::HeaderValue::from_str(&content_type) else {
        return invalid();
    };
    request
        .headers_mut()
        .insert(::http::header::CONTENT_TYPE, header);
    let Some(response) = execute_on_runtime(async move { schema.execute_http(request).await })
    else {
        return invalid();
    };
    let Ok(json) = String::from_utf8(response.body().clone()) else {
        return invalid();
    };
...
pub extern "C" fn response_is_ok(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(value) = response_value(args[0]) else {
        return invalid();
    };
    let is_ok = value
        .get("errors")
...
pub extern "C" fn response_errors_json(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Ok(encoded) = serde_json::to_string(&errors) else {
        return invalid();
    };
...
pub extern "C" fn response_drop(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    if store.responses.remove(args[0]).is_some() {
        write_result(ctx, 1)
    } else {
        invalid()
    }
...
pub extern "C" fn subscribe(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(subscription) = open_subscription(schema, request) else {
        return invalid();
    };
    let mut store = store()
        .lock()
    ...
    let Some(handle) = store
        .subscriptions
        .insert(std::sync::Arc::new(subscription))
    else {
        return invalid();
    };
    write_result(ctx, handle)
...
pub extern "C" fn subscription_next(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let Some(response) = execute_on_runtime(async move { subscription.next().await }) else {
        return invalid();
    };
...
pub extern "C" fn subscription_pending(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let Some(subscription) = store.subscriptions.get(args[0]) else {
        return invalid();
    };
...
pub extern "C" fn subscription_drop(ctx: *mut SpectraHostCallContext) -> i32 {
    ...
    let mut store = store()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    if store.subscriptions.remove(args[0]).is_some() {
        write_result(ctx, 1)
    } else {
        invalid()
    }
