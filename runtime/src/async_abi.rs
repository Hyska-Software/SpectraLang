...
use crate::ffi::{SpectraHostValue, HOST_STATUS_INVALID_ARGUMENT, HOST_STATUS_NOT_FOUND, HOST_STATUS_SUCCESS};
use crate::stdlib::{
...
fn frames() -> &'static Mutex<HashMap<SpectraHostValue, FrameOwner>> { &FRAME_OWNERS }
fn status(code: i32) -> i64 { code as i64 }

#[no_mangle]
pub extern "C" fn spectra_rt_coroutine_frame_alloc(slot_count: i64) -> i64 {
    let Ok(slot_count) = usize::try_from(slot_count) else { return 0 };
    if slot_count > 65_536 { return 0; }
    ...
    if guard.insert(ptr, FrameOwner::Pending(frame)).is_some() { return 0; }
    ptr
}

#[no_mangle]
pub extern "C" fn spectra_rt_coroutine_frame_store(frame_ptr: i64, slot: i64, value: i64) -> i64 {
    let Ok(slot) = usize::try_from(slot) else { return status(HOST_STATUS_INVALID_ARGUMENT) };
    let mut guard = frames().lock().unwrap_or_else(|poison| poison.into_inner());
    if let Some(FrameOwner::Pending(frame)) = guard.get_mut(&frame_ptr) {
        return status(if frame.store_slot(slot, value) { HOST_STATUS_SUCCESS } else { HOST_STATUS_INVALID_ARGUMENT });
    }
    drop(guard);
    ...
    status(if registry.store_slot_ptr(frame_ptr, slot, value) { HOST_STATUS_SUCCESS } else { HOST_STATUS_NOT_FOUND })
}

#[no_mangle]
pub extern "C" fn spectra_rt_coroutine_frame_load(frame_ptr: i64, slot: i64) -> i64 {
    let Ok(slot) = usize::try_from(slot) else { return -1 };
    let mut guard = frames().lock().unwrap_or_else(|poison| poison.into_inner());
    if let Some(FrameOwner::Pending(frame)) = guard.get_mut(&frame_ptr) {
...
pub extern "C" fn spectra_rt_coroutine_state_store(frame_ptr: i64, state: i64) -> i64 {
    let Ok(state) = u32::try_from(state) else { return status(HOST_STATUS_INVALID_ARGUMENT) };
    let mut guard = frames().lock().unwrap_or_else(|poison| poison.into_inner());
    if let Some(FrameOwner::Pending(frame)) = guard.get_mut(&frame_ptr) {
    ...
    status(if registry.store_state_ptr(frame_ptr, state) { HOST_STATUS_SUCCESS } else { HOST_STATUS_NOT_FOUND })
}

#[no_mangle]
pub extern "C" fn spectra_rt_coroutine_create(frame_ptr: i64, poll_ptr: i64, drop_ptr: i64) -> i64 {
    if frame_ptr == 0 || poll_ptr == 0 || drop_ptr == 0 { return 0; }
    let mut frame = {
        let mut guard = frames().lock().unwrap_or_else(|poison| poison.into_inner());
    ...
    let poll = unsafe { std::mem::transmute(poll_ptr as usize) };
    let drop = unsafe { std::mem::transmute(drop_ptr as usize) };
...
pub extern "C" fn spectra_rt_coroutine_poll_child(task: i64) -> i64 {
    match poll_coroutine_task(task) {
        ...
        Ok(AsyncPollOutcome::Failed | AsyncPollOutcome::AlreadyPolling) => AsyncPollStatus::Failed as i64,
        Ok(AsyncPollOutcome::Cancelled) => AsyncPollStatus::Cancelled as i64,
        Ok(AsyncPollOutcome::Stale | AsyncPollOutcome::AffinityRejected) => AsyncPollStatus::Failed as i64,
        Err(HOST_STATUS_NOT_FOUND) => match poll_task_once(task) {
            ...
            Ok(false) => {
                let status = crate::stdlib::lock_async_task_registry()
                    .ok()
                    .and_then(|registry| registry.tasks.get(task).map(|task| {
                        if task.cancelled {
...
#[no_mangle]
pub extern "C" fn spectra_rt_coroutine_suspend(_task: i64, _state: i64) -> i64 { status(HOST_STATUS_SUCCESS) }
...
mod tests {
    ...
    fn frame_abi_validates_slots_and_stale_pointers() {
        ...
        assert_eq!(spectra_rt_coroutine_frame_store(frame, 1, 1), HOST_STATUS_INVALID_ARGUMENT as i64);
    ...
    fn attached_frame_keeps_pointer_identity_for_generated_operations() {
        ...
        let task = spectra_rt_coroutine_create(frame, ready_poll as usize as i64, ready_drop as usize as i64);
        assert_ne!(task, 0);
