...
fn option_result_unwrap_wrong_variant_returns_host_status() {
    ...
    assert_eq!(
        call_host(
            option_result::OPTION_UNWRAP,
            &[none.as_ptr() as SpectraHostValue]
        )
        .0,
        HOST_STATUS_INVALID_ARGUMENT
    );
    ...
    assert_eq!(
        call_host(
            option_result::RESULT_UNWRAP,
            &[err.as_ptr() as SpectraHostValue]
        )
        .0,
        HOST_STATUS_INVALID_ARGUMENT
    );
    assert_eq!(
        call_host(
            option_result::RESULT_UNWRAP_ERR,
            &[ok.as_ptr() as SpectraHostValue]
        )
        .0,
        HOST_STATUS_INVALID_ARGUMENT
    );
    ...
    assert_eq!(
        call_host(
            option_result::OPTION_UNWRAP,
            &[some.as_ptr() as SpectraHostValue]
        ),
...
fn fs_directory_surface_create_rename_copy_and_readdir() {
    ...
    assert_eq!(with_list_registry(|reg| reg.len(empty_handle)), Ok(0usize));
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert_eq!(unsafe { tagged_result_parts(tagged) }, (0, source_bytes));
    assert_eq!(
    ...
    assert_eq!(
        call_host(
            FS_CREATE_DIR_ALL,
            &[test_string(empty_child.to_string_lossy().as_ref())]
        )
        .0,
        HOST_STATUS_SUCCESS
    );
...
fn collections_list_lifecycle() {
    ...
    let free_all_fn = lookup_host_function(LIST_FREE_ALL).expect("list_free_all not registered");
...
fn tensor_native_apply_tanh_sqrt_matches_analytic_and_finite_difference() {
    ...
    let check_unary = |host: &str,
        ...
        assert_eq!(
            call_host(TENSOR_REQUIRES_GRAD, &[x, 1]).0,
            HOST_STATUS_SUCCESS
        );
        ...
        for (index, input) in inputs.iter().enumerate() {
            ...
            let finite = (forward(input + epsilon) - forward(input - epsilon)) / (2.0 * epsilon);
            assert!(
...
fn tensor_native_apply_bce_matches_analytic_and_finite_difference() {
    ...
    assert_eq!(
        call_host(TENSOR_REQUIRES_GRAD, &[pred, 1]).0,
        HOST_STATUS_SUCCESS
    );
...
fn tensor_native_apply_conv_pool_dropout_matches_reference() {
    ...
    let grad_of = |handle: SpectraHostValue| -> Vec<f64> {
        ...
        assert_eq!(status, HOST_STATUS_SUCCESS);
        ml_tensor_float_data(grad as usize).expect("grad values").1
    };
    ...
    let (status, conv) = call_host(ML_CONV2D, &[input, kernel, bias, 1, 1, 2, 2, 1, 2, 2]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert_eq!(
    ...
    let (status, pooled) = call_host(ML_MAX_POOL2D, &[pooled_in, 1, 1, 2, 2, 2, 2]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert_eq!(
    ...
    let pool_forward = |values: &[f64]| values.iter().copied().fold(f64::NEG_INFINITY, f64::max);
    for index in 0..4 {
        let mut plus = pool_values.clone();
        plus[index] += epsilon;
        let mut minus = pool_values.clone();
        minus[index] -= epsilon;
        let finite = (pool_forward(&plus) - pool_forward(&minus)) / (2.0 * epsilon);
    ...
    let (_, dropped_out, _) = ml_tensor_float_data(dropped as usize).expect("dropout output");
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (_, identical_out, _) = ml_tensor_float_data(identical as usize).expect("inference output");
...
fn tensor_native_apply_batched_and_shape_matches_reference() {
    ...
    let a = tensor_alloc(
        TensorDType::Float,
        vec![2, 2, 2],
        f64_values_to_host(&a_values),
    )
    .expect("alloc a") as SpectraHostValue;
    let b = tensor_alloc(
        TensorDType::Float,
        vec![2, 2, 2],
        f64_values_to_host(&b_values),
    )
    ...
    for (host, opcode, left_values, right_values) in [
        (
            "spectra.std.tensor.concat",
            24,
            vec![1.0, 2.0, 3.0],
            vec![4.0, 5.0],
        ),
        (
            "spectra.std.tensor.stack",
            25,
            vec![1.0, 2.0],
            vec![4.0, 5.0],
        ),
    ...
    assert_eq!(
        call_host(TENSOR_REQUIRES_GRAD, &[seq, 1]).0,
        HOST_STATUS_SUCCESS
    );
    let (status, window) = call_host(TENSOR_SLICE, &[seq, 1, 4]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let slice_up = tensor_alloc(TensorDType::Float, vec![3], f64_values_to_host(&[1.0; 3]))
    ...
    let mat = tensor_alloc(
        TensorDType::Float,
        vec![2, 3],
        f64_values_to_host(&mat_values),
    )
    .expect("alloc mat") as SpectraHostValue;
    assert_eq!(
        call_host(TENSOR_REQUIRES_GRAD, &[mat, 1]).0,
        HOST_STATUS_SUCCESS
    );
...
fn tensor_runtime_phase15_memory_report_tracks_lifetimes_sites_and_reuse() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let report = unsafe { read_spectra_string(report_ptr) }.expect("valid memory report string");
...
fn ml_phase17_experiment_tracking_manifests_compare_and_repro_command() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let manifest_a = unsafe { read_spectra_string(manifest_a_ptr) }.expect("manifest path string");
...
fn ml_phase18_onnx_subset_export_import_and_roundtrip() {
    ...
    let weight_list = |handles: Vec<SpectraHostValue>| -> SpectraHostValue {
        ...
        for handle in handles {
            assert_eq!(call_host(LIST_PUSH, &[list, handle]).0, HOST_STATUS_SUCCESS);
        }
        list
    };
    ...
    for (kind, expected_op, dims) in [
        ...
        let weights = weight_list(dims.into_iter().map(live_tensor).collect::<Vec<_>>());
        ...
        let (status, summary_ptr) = call_host(ML_ONNX_IMPORT_SUMMARY, &[test_string(&exported)]);
        assert_eq!(status, HOST_STATUS_SUCCESS);
        ...
        assert_eq!(status, HOST_STATUS_SUCCESS);
        let roundtrip_path = unsafe { read_spectra_string(roundtrip_ptr) }.expect("roundtrip path");
        assert_eq!(
...
fn ml_logits_sample_seeded_is_reproducible_and_diverges() {
    ...
    let sample =
        |seed: SpectraHostValue| call_host(ML_LOGITS_SAMPLE_SEEDED, &[seed, logits, temperature]);
...
fn ml_phase18_rag_tokenizer_vector_index_and_prompt_eval() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let loaded_query = unsafe { read_spectra_string(loaded_query_ptr) }.expect("loaded query json");
...
fn ml_phase19_evaluation_metrics_and_report() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (status, classification_ptr) = call_host(ML_METRICS_CLASSIFICATION, &[labels, predicted]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let classification =
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (status, regression_predicted) = call_host(TENSOR_FULL_F, &[4, 1.0f64.to_bits() as i64]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
...
fn tensor_runtime_gpu_parallel_sum_device_dispatch_matches_cpu() {
    use crate::gpu::{
        readback_scalar_device, sum_device, with_device_queue, DeviceArena, PoolDType, PoolDevice,
    };
...
fn tensor_runtime_gpu_parallel_sum_boundary_sizes_match_cpu() {
    use crate::gpu::{
        readback_scalar_device, reduction_plan, sum_device, with_device_queue, DeviceArena,
        PoolDType, PoolDevice,
    };
    ...
    let results = with_device_queue(
        |device, queue| -> Result<Vec<(usize, f64, f32)>, crate::gpu::GpuError> {
            let mut arena = DeviceArena::new();
            let mut out = Vec::new();
            for &n in &sizes {
...
fn concurrent_tasks_can_join_in_reverse_creation_order() {
    ...
    for value in 1..=10 {
        let closure = heap_closure(spawn_fn_identity_closure as *const () as usize, 0);
...
fn async_task_wait_reports_ready_and_cancelled_without_spinning() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert_eq!(
        call_host(ASYNC_TASK_CANCEL, &[cancelled_task]).0,
        HOST_STATUS_SUCCESS
    );
    assert_eq!(
...
fn async_stdlib_host_calls_cover_fs_tcp_udp_channels_and_cancellation() {
    ...
    let (status, cancelled_write) = call_host(ASYNC_FS_WRITE, &[file_arg, test_string("cancel")]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert_eq!(
    ...
    assert!(
        client_stream > 0,
        "async TCP connect returned {client_stream}"
    );
    let (status, server_stream) = call_host(ASYNC_TASK_BLOCK_ON, &[accept_task]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert!(
        server_stream > 0,
        "async TCP accept returned {server_stream}"
    );
    assert_eq!(
        call_host(ASYNC_REACTOR_STATS_IO_REGISTRATIONS, &[]),
        (HOST_STATUS_SUCCESS, 3)
...
fn substring_byte_semantics_multibyte_boundaries() {
    ...
    let substring = |start: i64, end: i64| {
        ...
        (
            status,
            unsafe { read_spectra_string(ptr) }.expect("substring must return a readable string"),
        )
    };
    ...
    assert_eq!(substring(1, 99), (HOST_STATUS_SUCCESS, "éllo".to_string()));
...
fn char_at_returns_bytes_and_rejects_invalid_indexes() {
    ...
    let s = test_string("h\u{e9}llo");
    assert_eq!(
        call_host(STR_CHAR_AT, &[s, 0]),
        (HOST_STATUS_SUCCESS, b'h' as i64)
    );
    assert_eq!(
        call_host(STR_CHAR_AT, &[s, 1]),
        (HOST_STATUS_SUCCESS, 0xC3_i64)
    );
    assert_eq!(
        call_host(STR_CHAR_AT, &[s, 3]),
        (HOST_STATUS_SUCCESS, b'l' as i64)
    );
...
fn ml_onnx_exported_linear_weights_run_matches_manual_computation() {
    ...
    assert!(weights > 0);
    assert_eq!(
        call_host(LIST_PUSH, &[weights, weight]).0,
        HOST_STATUS_SUCCESS
    );
    assert_eq!(
        call_host(LIST_PUSH, &[weights, bias]).0,
        HOST_STATUS_SUCCESS
    );
    let (status, exported_ptr) = call_host(
        ML_ONNX_EXPORT_WEIGHTS,
    ...
    let (status, output) = call_host(ML_ONNX_RUN, &[session, input, test_string("output")]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let expected: Vec<f64> = (0..3)
        .map(|j| 0.75 * weight_values[j] + -1.25 * weight_values[3 + j] + bias_values[j])
        .collect();
    ...
    assert_eq!(
        call_host(
            ML_ONNX_RUN,
            &[session + 9_999, input, test_string("output")]
        )
        .0,
        HOST_STATUS_NOT_FOUND
    );
...
fn ml_onnx_export_weights_serializes_live_tensors_and_validates() {
    ...
    assert_eq!(
        call_host(LIST_PUSH, &[short, weight]).0,
        HOST_STATUS_SUCCESS
    );
    assert_eq!(
        call_host(
    ...
    .expect("alloc bad-shape weight") as SpectraHostValue;
    ...
    assert_eq!(
        call_host(LIST_PUSH, &[bad_list, bias]).0,
        HOST_STATUS_SUCCESS
    );
    assert_eq!(
        call_host(
    ...
    assert_eq!(
        call_host(
            ML_ONNX_EXPORT_WEIGHTS,
            &[test_string(&path_str), test_string("linear"), 999_999]
        )
        .0,
        HOST_STATUS_NOT_FOUND
    );
...
fn ml_onnx_export_weights_roundtrip_matches_trained_forward() {
    ...
    for _ in 0..500 {
        ...
        for (input, target) in &samples {
            for j in 0..3 {
                let out = input[0] * weight[j] + input[1] * weight[3 + j] + bias[j];
    ...
    let path_str = path.to_string_lossy().to_string();
    let weight_handle = tensor_alloc(TensorDType::Float, vec![2, 3], f64_values_to_host(&weight))
        .expect("alloc trained weight") as SpectraHostValue;
    let bias_handle = tensor_alloc(TensorDType::Float, vec![3], f64_values_to_host(&bias))
    ...
    let (status, session) = call_host(ML_ONNX_SESSION_FROM_BYTES, &[test_string(&path_str)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert!(session > 0);

    let probe = [0.5f64, -1.5f64];
    let input = tensor_alloc(TensorDType::Float, vec![1, 2], f64_values_to_host(&probe))
...
fn ml_onnx_roundtrip_summary_reflects_real_session_metadata() {
    ...
    let summary = unsafe { read_spectra_string(summary_ptr) }.expect("summary");
    assert!(
        summary.contains("\"metadata\":\"onnxruntime-session\""),
        "{summary}"
    );
    ...
    assert!(
        summary.contains(
            "\"input_details\":[{\"name\":\"input\",\"dtype\":\"float32\",\"shape\":[1,2]}]"
        ),
        "{summary}"
    );
    assert!(
        summary.contains(
            "\"output_details\":[{\"name\":\"output\",\"dtype\":\"float32\",\"shape\":[1,3]}]"
        ),
        "{summary}"
    );
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let roundtrip_path = unsafe { read_spectra_string(roundtrip_ptr) }.expect("roundtrip path");
    let (status, session) = call_host(ML_ONNX_SESSION_FROM_BYTES, &[test_string(&roundtrip_path)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert!(session > 0);
    assert_eq!(
        call_host(ML_ONNX_SESSION_FREE, &[session]),
...
fn ml_onnx_run_without_feature_returns_typed_error() {
    ...
    let (run_status, run_tagged) = call_host(ML_ONNX_RUN, &[1, 1, test_string("output")]);
...
fn ml_tokenizer_training_bpe_learns_expected_merges_and_roundtrips() {
    ...
    let tokens: Vec<&str> = lines_a
        ...
        .collect();
    for expected in [
        "[UNK]", "es", "##es", "est", "##est", "lo", "##lo", "low", "##low", "ewest", "##ewest",
        "newest", "##newest", "widest", "##widest", "lower", "##lower",
    ] {
        assert!(
    ...
    let (status, tokenizer) = call_host(ML_TOKENIZER_TRAIN_BPE, &[test_string(corpus), 64]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (status, ids) = call_host(
        ML_TOKENIZER_ENCODE,
...
fn ml_tokenizer_training_wordpiece_scores_base_coverage_and_roundtrip() {
    ...
    let lines = trained_vocab_lines("wordpiece", ML_TOKENIZER_TRAIN_WORDPIECE, corpus, 400);
    ...
    assert!(
        tokens
            .iter()
            .any(|token| { token.starts_with("##") && token.chars().count() > 3 }),
        "no multi-char continuation learned; got {tokens:?}"
    );

    // Encode a corpus sentence: no UNK anywhere, decode restores it.
    let (status, tokenizer) = call_host(ML_TOKENIZER_TRAIN_WORDPIECE, &[test_string(corpus), 400]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let sentence = "o rei de roma";
    let (status, ids) = call_host(ML_TOKENIZER_ENCODE, &[tokenizer, test_string(sentence)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
...
fn ml_tokenizer_training_ptbr_corpus_encodes_without_unk() {
    ...
    for (host, name) in [
        ...
        let (status, tokenizer) = call_host(host, &[test_string(corpus), 300]);
...
fn ml_tokenizer_training_artifact_save_load_is_idempotent() {
    ...
    let (status, handle_a) = call_host(ML_TOKENIZER_TRAIN_BPE, &[test_string(corpus), 64]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let build_and_save = |save_path: String| -> SpectraHostValue {
        let (status, artifact) = call_host(
            ML_ARTIFACT_NEW,
            &[
                test_string("trained-tokenizer"),
                test_string("v1"),
                test_string("multi_array"),
            ],
        );
        assert_eq!(status, HOST_STATUS_SUCCESS);
        for (key, value) in [("tokenizer_type", "wordpiece"), ("tokenizer_version", "v1")] {
            let (status, _) = call_host(
    ...
    let (status, handle_c) = call_host(
        ML_TOKENIZER_LOAD,
        &[test_string(second_path.to_string_lossy().as_ref())],
    );
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    for index in 0..len {
        assert_eq!(
            call_host(TENSOR_GET, &[ids_a, index]).1,
            call_host(TENSOR_GET, &[ids_b, index]).1
        );
        assert_eq!(
            call_host(TENSOR_GET, &[ids_a, index]).1,
            call_host(TENSOR_GET, &[ids_c, index]).1
        );
    }
...
fn ml_metrics_generation_real_perplexity_from_logprobs() {
    ...
    let (status, plain_ptr) = call_host(
        ML_METRICS_GENERATION,
        &[test_string("alpha beta"), test_string("alpha gamma")],
    );
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let (status, logprobs) = call_host(TENSOR_LITERAL_F, &[2, bits[0], bits[1]]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
...
fn ml_text_embed_model_matches_manual_masked_pooling() {
    ...
    std::fs::write(&model_path, ml_text_embed_fixture_proto()).expect("write fixture model");
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (_, values, _) = ml_tensor_float_data(out as usize).expect("embedding tensor data");
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (_, values_again, _) = ml_tensor_float_data(out_again as usize).expect("second embedding");
    ...
    let (status, out_masked) = call_host(
        ML_TEXT_EMBED_MODEL,
        &[session, tokenizer, test_string("hello world deep"), mask],
    );
    assert_eq!(status, HOST_STATUS_SUCCESS);
...
fn ml_disttcp_protocol_frame_roundtrip_byte_by_byte() {
    ...
    let decoded = dist_decode_gradients(&gradients[ML_DIST_FRAME_HEADER_LEN..consumed])
    ...
    assert_eq!(
        i64::from_le_bytes(payload[0..8].try_into().expect("step")),
        41
    );
...
fn ml_disttcp_multithread_four_workers_disjoint_shards_converge() {
    ...
    let loss_start = summary.find("\"last_loss\":").expect("loss field") + "\"last_loss\":".len();
    let loss_end = loss_start
        + summary[loss_start..]
            .find(',')
            .expect("loss field terminator");
    let last_loss: f64 = summary[loss_start..loss_end]
        .trim()
        .parse()
        .expect("loss value");
    assert!(
...
pub(crate) fn serve_real_linear_forward_matches_manual_reference() {
    ...
    let (status, vector_handle) = call_host(SERVE_SERVER_RESULT_VECTOR, &[server, request]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (_, values, _) = ml_tensor_float_data(vector_handle as usize).expect("output vector");
    let expected_second = (-9.25f64).tanh();
    assert_eq!(values.len(), 2);
    assert!(
        (values[0] - expected_first).abs() < 1e-9,
        "got {}",
        values[0]
    );
    assert!(
        (values[1] - expected_second).abs() < 1e-9,
        "got {}",
        values[1]
    );
}
...
pub(crate) fn serve_real_latency_measured_positive_and_grows_with_compute() {
    ...
    heavy_w2_args.extend(
        (0..160_000).map(|index| serve_real_bits(if index % 7 == 0 { 0.125 } else { 0.0 })),
    );
...
fn heap_closure(code_ptr: usize, _capture: i64) -> SpectraHostValue {
    Box::into_raw(Box::new([code_ptr as SpectraHostValue, _capture])) as SpectraHostValue
}
...
fn ml_generate_autoregressive_greedy_matches_fixture_pattern() {
    ...
    let make_ids = |ids: &[i64]| -> SpectraHostValue {
        tensor_alloc(TensorDType::Int, vec![ids.len()], ids.to_vec()).expect("alloc prompt tensor")
            as SpectraHostValue
    };
...
fn ml_generate_ex_seed_is_reproducible_and_temperature_changes_distribution() {
    ...
    let input = make_ids(&[0]);
    assert_eq!(
        call_host(ML_GENERATE_EX, &[session, input, 4, -1, bits(-1.0), 64, 0]).0,
        HOST_STATUS_INVALID_ARGUMENT
    );
...
fn ml_onnx_run_multi_accepts_symbolic_dims_and_matches_feed() {
    ...
    let feed = |shape: Vec<usize>, values: &[f64]| -> usize {
        ...
        assert_eq!(
            status, HOST_STATUS_SUCCESS,
            "symbolic dims must be accepted"
        );
        output as usize
    };
...
fn serve_multi_model_alternates_named_models_and_reports_per_model_metrics() {
    ...
    let (status, w_double) = call_host(TENSOR_LITERAL2_F, &[1, 1, serve_real_bits(2.0)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let (status, w_shift) = call_host(TENSOR_LITERAL2_F, &[1, 1, serve_real_bits(3.0)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let direct_infer = |model: &str, x: f64| -> SpectraHostValue {
        let (status, input) = call_host(TENSOR_LITERAL_F, &[1, serve_real_bits(x)]);
        assert_eq!(status, HOST_STATUS_SUCCESS);
        let (status, request) = call_host(SERVE_SERVER_INFER, &[server, test_string(model), input]);
    ...
    let (_, vec_handle) = call_host(SERVE_SERVER_RESULT_VECTOR, &[server, r_shift_a]);
    let (_, values, _) = ml_tensor_float_data(vec_handle as usize).expect("output vector");
    assert!(
        (values[0] - (9.25f64).tanh()).abs() < 1e-9,
        "got {}",
        values[0]
    );
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let snapshot = unsafe { read_spectra_string(snapshot_ptr) }.expect("snapshot");
    let parsed: serde_json::Value = serde_json::from_str(&snapshot).expect("snapshot JSON");
...
fn serve_multi_model_vector_request_indim4_matches_manual_math() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (status, request) = call_host(SERVE_SERVER_INFER, &[server, test_string("vec4"), input]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (_, vec_handle) = call_host(SERVE_SERVER_RESULT_VECTOR, &[server, request]);
    let (shape, values, _) = ml_tensor_float_data(vec_handle as usize).expect("output vector");
...
fn serve_http_listener_routes_named_models_and_vector_inputs() {
    ...
    serve_real_register_scalar_model(server, 2.0);
    let (status, w_triple) = call_host(TENSOR_LITERAL2_F, &[1, 1, serve_real_bits(3.0)]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let response =
        serve_http_request_once(port, "GET /metrics HTTP/1.1\r\nHost: localhost\r\n", "");
    assert!(response.starts_with("HTTP/1.1 200"), "{response}");
...
fn serve_http_request_once(port: u16, request_head: &str, body: &str) -> String {
    ...
    let mut stream =
        std::net::TcpStream::connect(("127.0.0.1", port)).expect("connect to embedded listener");
    stream
        .write_all(
            format!(
                "{}Content-Length: {}\r\nConnection: close\r\n\r\n{}",
...
fn serve_http_embedded_listener_serves_infer_and_metrics_then_stops() {
    ...
    assert_eq!(snapshot["schema"], "spectra.serve.monitoring_snapshot.v1");
    assert!(
    ...
    assert_eq!(
        call_host(SERVE_HTTP_STOP, &[server]).0,
        HOST_STATUS_INVALID_ARGUMENT
    );
}
...
fn ml_onnx_run_multi_feeds_two_graph_inputs_and_matches_manual_add() {
    ...
    let (status, tagged) = call_host(ML_ONNX_RUN_MULTI, &[session, bad_names, tensors_again]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (tag, error) = unsafe { tagged_result_parts(tagged) };
    ...
    let (status, tagged) = call_host(ML_ONNX_RUN_MULTI, &[session, shape_names, shape_tensors]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    ...
    let (message_status, message_ptr) = call_host("spectra.std.error.message", &[shape_error]);
    assert_eq!(message_status, HOST_STATUS_SUCCESS);
    let message = unsafe { read_spectra_string(message_ptr) }.expect("error message");
    assert!(
    ...
    let (status, tagged) = call_host(ML_ONNX_RUN_MULTI, &[session, short_names, short_tensors]);
    assert_eq!(status, HOST_STATUS_SUCCESS);
    let (short_tag, short_error) = unsafe { tagged_result_parts(tagged) };
    assert_eq!(short_tag, 1);
    let (message_status, message_ptr) = call_host("spectra.std.error.message", &[short_error]);
    assert_eq!(message_status, HOST_STATUS_SUCCESS);
    let message = unsafe { read_spectra_string(message_ptr) }.expect("error message");
    ...
    let (status, one_output) =
        call_host(ML_ONNX_RUN_MULTI, &[single_session, one_names, one_tensors]);
    assert_eq!(
        status, HOST_STATUS_SUCCESS,
        "single-input model through the multi host must succeed"
    );
    assert!(one_output > 0);
    let (_, one_values, _) = ml_tensor_float_data(one_output as usize).expect("one-output data");
...
fn tensor_runtime_gpu_residency_reduce_ops_match_cpu() {
    ...
    assert_eq!(status, HOST_STATUS_SUCCESS);
    assert!((f64::from_bits(mean_cpu_bits as u64) - f64::from_bits(mean_bits as u64)).abs() < 1e-5,);
    let _ = call_host(TENSOR_FREE_ALL, &[]);
}
...
fn tensor_runtime_gpu_float_unary_matches_cpu() {
    ...
    for (op, reference) in cases {
        ...
        for (index, value) in values.iter().enumerate() {
            ...
            assert!(
                (got - f64::from(expected)).abs() <= 1e-5 * f64::from(expected).abs().max(1.0),
                "{op}[{index}] {got} vs cpu {}",
                reference(*value)
            );
        }
...
fn tensor_runtime_r3052_device_grad_without_gpu_backward_uses_cpu_readback() {
    ...
    let alloc_device_leaf = |shape: Vec<usize>, values: &[f64]| -> Result<usize, i32> {
    ...
    let cpu_i = tensor_alloc_autograd(TensorDType::Float, vec![4], input_bits.clone(), true, None)
