...
- The required 558-name `spectra.api.*` namespace lives in the canonical `packages/spectra-api/src/host_calls.rs` registry (`runtime/src/api/mod.rs` was removed as a stale duplicate).
