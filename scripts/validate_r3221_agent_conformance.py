...
The checks run in order, stopping at the first failure:
  ...
  (f) the verification fixtures (`tests/validation/384..432`) in JIT and AOT --
      ...
      the stream teardown boundary, provider routing, the scripted directive
      grammar, the async scalar-slot code-generation regressions, memory
      boundaries, ceiling combinations, replay divergence, A2A task states,
      taint ledger tags, nested payloads, schema recovery, payload privacy,
      tool replay, stream interleaving, memory ties, capability boundaries,
      compensation failure, MCP replay, protocol negatives, taint policy
...
EXAMPLES = {
    ...
    "38-cross-module-tools": "examples/agent/38-cross-module-tools",
    "39-remote-scope-isolation": "examples/agent/39-remote-scope-isolation",
...
VERIFICATION_FIXTURES = {
    ...
    "remote_scope": "tests/validation/429_agent_remote_scope.spectra",
...
def check_examples() -> dict:
    ...
    def check(entry: tuple[str, str]) -> tuple[str, dict]:
        ...
        code, output = run([binary, "run", project], timeout=600)
        if code != 0:
